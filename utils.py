...
class RenderWrapper(gym.core.Wrapper):
    def __init__(self, env, render_mode='screen', track_object='robot', hide_grid=False):
        super().__init__(env)
        #self.viewer = EvoViewer(env.env.env.env.sim, resolution=(2000, 2000), target_rps=120)
        self.viewer = EvoViewer(env.sim, resolution=(2100, 2100), target_rps=120)
    ...
    def step(self, action):
        ...
        if self.render_mode_ == 'screen':
            self.viewer.render(self.render_mode_)
        elif self.render_mode_ == 'img':
            self.imgs.append(self.viewer.render(self.render_mode_, hide_grid=self.hide_grid))
        else:
