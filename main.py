...
# sim
parser.add_argument('--sim_sif', '-ss', type=str, default='evogym-work.sif', 
                    choices=['evogym-work.sif'], help='simulation sif')
# task 
parser.add_argument('--task', '-t', help='specify the task',
                    choices=['MnistEnv-v0', 'MnistEnv-v1'], default='MnistEnv-v0') # v0: 0;1 -- v1: 0,1,2,3 -- v2: 0;5 -- v3: 0,1;5,6 -- v4: 0,1,2;5,6,7 and so on
...
parser.add_argument('-sq', '--slurm_queue', type=str,
                    help='choose the job queue for the slurm submissions', choices=['short', 'week', 'general'])
...
def submit_slurm(args, resubmit=False):
    ...
    elif args.slurm_queue == 'general':
        base_string += '#SBATCH --time=0-29:00:00  # Request 29 hours of runtime \n\n'

    base_string += 'cd /users/a/m/amertan/workspace/EVOGYM/evogym-mnist/ \n'
    base_string += 'module load apptainer\n'
    #base_string += 'trap \'kill -INT "${PID}"; wait "${PID}"; handler\' USR1 SIGINT SIGTERM \n'
    base_string += 'apptainer exec --bind ../../../scratch/evogym_experiments:/scratch_evogym_experiments ' + args.sim_sif + ' xvfb-run -a /evogym-venv/bin/python '
